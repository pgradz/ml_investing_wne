...
def transformer_encoder(inputs, head_size, num_heads, ff_dim, dropout=0):
    ...
    x = layers.Conv1D(filters=40, kernel_size=5, activation="relu",  padding='same')(x)
    x = layers.Dropout(dropout)(x)
    return x + res
...
#     inputs = keras.Input(shape=input_shape)
#     x = inputs
#     x2vec = Time2Vec(40)(inputs)
#     x = layers.concatenate([x, x2vec])
